...
这里是一个简单的 CUDA Softmax 算子示例。

当前包含：

- `v0.cu`：naive softmax，每个 CUDA block 处理一行
- `softmax_config.h`：默认输入尺寸、block size 和结果检查函数
...
```text
Y[row, col] = exp(X[row, col] - max(X[row, :])) / sum(exp(X[row, :] - max(X[row, :])))
```
...
```bash
cd /mlx_devbox/users/yeliming.void/playground/ai-infra/code/softmax
```
...
- `bandwidth`：按两次读取 `X`、一次写入并一次读写 `Y` 估算的有效带宽
